...
const originalTheme = require("../original/dark-modern.json");
...
for (let [property, color] of Object.entries(originalTheme.colors)) {
  if (color && isGrayscale(hexToHsl(color.slice(0, 7).toString()))) {
    theme.colors[property] = color;
  }
}

for (let [property, original] of Object.entries(theme.colors)) {
  const color = original.slice(0, 7);
  const alpha = original.slice(7) ?? "";
  ...
  if (isGrayscale(originalHsl)) {
    ...
    theme.colors[property] = hslToHex(alteredHsl) + alpha;
  }
}
